...
from epex.tools.base import ToolResult
from epex.core.security import SecurityGuardian, GuardrailEngine
...
from epex.core.modes import ModeManager, EpexMode
from epex.core.roles import RoleManager
...
class TaskManager:
    ...
    def __init__(self, llm_router=None, security_guardian=None, soul=None):
        ...
        self.task_checkpoints: Dict[str, List[Dict[str, Any]]] = {} # task_id -> list of states
        self.shadow_tasks: List[str] = [] # List of task IDs running in "Shadow Autonomy" mode
    ...
    async def create_task_from_command(self, command: str) -> Task:
        async with AsyncSessionLocal() as session:
            ...
            from epex.models.core import User
            from sqlalchemy import select
            result = await session.execute(select(User).limit(1))
            user = result.scalars().first()
            if not user:
                user = User(username="default", email="default@epex.bot", password_hash="none")
                session.add(user)
    ...
    async def _execute_step(self, step: Dict[str, Any], task: Task) -> ToolResult:
        ...
        if self.mode_manager.current_mode == EpexMode.SHADOW:
