...
class OmniSearchTool(Tool):
    ...
    description = "Search across local files, the web, and all connected Epex peers simultaneously."
    category = "web"
    risk_level = "low"
    parameters = {
    ...
    async def _simulated_web_search(self, query: str):
        ...
        return [
            {"title": f"Web result for {query}", "url": "https://epex.ai/docs", "snippet": "Found relevant info on the web."}
        ]
