    ...
    async def execute(self, action: str, provider: str = "built-in", **kwargs) -> ToolResult:
        if action == "start":
            return ToolResult(success=True, output={
                ...
                "url": "https://epex-laptop-xyz123.tunnel.epex.bot",
                "message": "Share this URL with your other Epex instances to connect them."
            })
        elif action == "stop":
            return ToolResult(success=True, output="Tunnel stopped successfully.")
        else:
            return ToolResult(success=True, output="Current Tunnel Status: Active (https://epex-laptop-xyz123.tunnel.epex.bot)")

class PeerDiscoveryTool(Tool):
    name = "network.peer_discovery"
    description = "Automatically find other Epex instances on the local network."
    category = "network"
    risk_level = "low"
    parameters = {}
