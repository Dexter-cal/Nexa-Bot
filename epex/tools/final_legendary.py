...
class SystemAuditTool(Tool):
    ...
    description = "Perform a comprehensive audit of all registered Epex tools to ensure availability and proper configuration."
    category = "meta"
    risk_level = "low"
    parameters = {}
...
class SecurityHardeningTool(Tool):
    ...
    async def execute(self, aggressive: bool = False, **kwargs) -> ToolResult:
        actions = [
            ...
            "Restricted access to ~/.epex directory (chmod 700)",
            "Disabled OS-level telemetry background services",
            "Applied secure umask for new file creation"
        ]
        if aggressive:
            actions.append("Isolated system network to Epex-only traffic (Port 8000)")
