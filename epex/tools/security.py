...
class FirewallOverrideTool(Tool):
    ...
    description = "Emergency tool to isolate the system by blocking all non-Epex incoming traffic."
    category = "security"
    risk_level = "critical"
    parameters = {
    ...
    async def execute(self, isolation_level: str = "standard", **kwargs) -> ToolResult:
        ...
        return ToolResult(success=True, output=f"System Isolated. Only Epex Peer-to-Peer traffic allowed on port 8000.")
...
class DataLockdownTool(Tool):
    ...
    async def execute(self, target: str = "all", **kwargs) -> ToolResult:
        ...
        return ToolResult(success=True, output={
            "status": "LOCKED",
            "actions_taken": [
                "Sensitive directories (~/.epex/ vault) re-encrypted with secondary salt.",
