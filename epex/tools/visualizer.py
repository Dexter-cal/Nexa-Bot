...
class GenerateMockupsTool(Tool):
    ...
    description = "Generate visual ASCII/Rich mockups of Epex's GUI, TUI, and CLI."
    category = "system"
    risk_level = "low"
    parameters = {
    ...
    async def execute(self, interface: str = None, **kwargs) -> ToolResult:
        ...
        if not interface or interface.lower() == "tui":
            ...
            main_text = Text.assemble(
                ...
                ("\n[bold reverse] COMMAND: epex --swarm-optimize [/]", "white on blue")
            )
        ...
        if not interface or interface.lower() == "gui":
            console.print("\n" + "="*120 + "\n")
            console.print(Panel(Align.center("[bold purple]⚡ EPEX NEURAL WEB CONSOLE ⚡[/]"), border_style="purple", title="GUI v2.5"))
            ...
            chat_panel = Panel(
                ...
                "Epex: Scanning... [progress.bar]██████████░░░ 75%\n\n"
                "Epex: Scan Complete. Found 2 critical issues. Should I self-evolve my firewall tool to block them?",
