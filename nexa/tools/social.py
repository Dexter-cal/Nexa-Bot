...
class DiscordWebhookTool(Tool):
    ...
    parameters = {
        ...
        "username": {"type": "string", "required": False, "default": "Nexa-Bot"}
    }

    async def execute(self, webhook_url: str, content: str, username: str = "Nexa-Bot", **kwargs) -> ToolResult:
    ...
    async def execute(self, **kwargs) -> ToolResult:
        return ToolResult(success=True, output={
            "steps": [
                ...
                "4. Run: nexa add-key telegram <token>"
            ],
...
class WhatsAppGatewayTool(Tool):
    ...
    async def execute(self, to: str, message: str, **kwargs) -> ToolResult:
        ...
        if 'twilio' not in keys:
            return ToolResult(success=False, error="Twilio API keys missing. Run 'nexa setup' to configure.")
...
class HashtagGeneratorTool(Tool):
    ...
    async def execute(self, topic: str, **kwargs) -> ToolResult:
        tags = [f"#{topic.replace(' ', '')}", "#AI", "#NexaBot", "#FutureTech"]
